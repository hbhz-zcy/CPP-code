...
#define nowtm clock()
#define k_d(VK_NONAME) ((GetAsyncKeyState(VK_NONAME) & 0x8000) ? 1:0)
int ID,q,t,data_N,data_M,data_a[100][100];
...
int main(){
	...
	if(ID==1){
		filew('w',3);
		filew('0');filer('0');
		printf("please enter number of points(1~50) and sides\n");
		scanf("%d%d",data_N,data_M);
		dataget();
		printf("OK!\n");
		Sleep(500);
		clear();
